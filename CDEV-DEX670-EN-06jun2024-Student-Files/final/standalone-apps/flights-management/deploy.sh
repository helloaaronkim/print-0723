...
if [ "$isRelease" == "true" ]; then
	...
	case $ENV in 
		dev)
			...
			./deploy-to-ch.sh $ENV "-$ENV""x" "0.1" $APCID $APSECRET $ENCRYPTKEY $APCACID $APCASECRET "nonprod-internalps"
			;;
		test)
			echo "Deploying $UNIT to $ENV"
			./deploy-to-ch.sh $ENV "-$ENV" "0.1" $APCID $APSECRET $ENCRYPTKEY $APCACID $APCASECRET "nonprod-internalps"
			;;
		prod)
			echo "Deploying $UNIT to $ENV"
			./deploy-to-ch.sh $ENV ''      "0.1" $APCID $APSECRET $ENCRYPTKEY $APCACID $APCASECRET "prod-internalps"
			;;
		*)
