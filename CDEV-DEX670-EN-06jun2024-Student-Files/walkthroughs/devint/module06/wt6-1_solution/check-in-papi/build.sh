...
../apps-commons/build.sh "$ENCRYPTKEY" "$CI_BUILD" true

../resilience-mule-extension/build.sh "$ENCRYPTKEY" "$CI_BUILD" true
...
if [ "$CI_BUILD" != "true" ]; then
	...
	$mvns verify -Dencrypt.key=$ENCRYPTKEY -Dmunit.runner.server.time.out=300000
else
	...
	$mvns verify -Dencrypt.key=$ENCRYPTKEY -Dmunit.runner.server.time.out=300000 $skipTests
